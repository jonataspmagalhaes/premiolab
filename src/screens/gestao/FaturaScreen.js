...
import { Glass, Pill, SectionLabel, EmptyState, SwipeableRow } from '../../components';
...
function calcPontos(movs, regras, tipoBeneficio, moedaCartao) {
  var total = 0;
  for (var i = 0; i < movs.length; i++) {
    var m = movs[i];
    if (m.tipo !== 'saida') continue;
    var val = m.valor || 0;
    var moedaGasto = m.moeda_original || moedaCartao || 'BRL';
    var matched = null;
    for (var j = 0; j < regras.length; j++) {
      var r = regras[j];
      if (r.moeda && r.moeda !== moedaGasto) continue;
      if (val >= (r.valor_min || 0) && (!r.valor_max || val <= r.valor_max)) {
        if (!matched || (r.moeda && !matched.moeda)) matched = r;
      }
    }
    if (matched) {
      if (tipoBeneficio === 'pontos') {
        total += val * matched.taxa;
      } else {
        total += val * (matched.taxa / 100);
      }
    }
  }
  return total;
...
export default function FaturaScreen(props) {
  ...
  // ── Current month ──
  var now = new Date();
  var _mes = useState(now.getMonth() + 1); var mes = _mes[0]; var setMes = _mes[1];
  var _ano = useState(now.getFullYear()); var ano = _ano[0]; var setAno = _ano[1];
  ...
  var load = function() {
    ...
    return Promise.all([
      ...
      if (faturaResult.data) {
        setPayValor(fmt(faturaResult.data.total));
      }
      setLoading(false);
    }).catch(function(err) {
  ...
  function handleSaveManual() {
    ...
    var descFatura = manualDesc.trim() || ('Fatura ' + MESES[mes - 1] + '/' + ano);
    var now = new Date();
    var dataStr = now.getFullYear() + '-' + pad2(now.getMonth() + 1) + '-' + pad2(now.getDate());
    ...
    addMovimentacaoCartao(user.id, mov).then(function(res) {
      ...
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Toast.show({ type: 'success', text1: 'Fatura lançada', text2: symb + ' ' + fmt(val) });
      setShowManual(false);
      setManualValor('');
      setManualDesc('');
      load();
  ...
  if (hasBeneficio) {
    pontosValue = calcPontos(movs, regras, cartao.tipo_beneficio, moedaCartao);
  }
      ...
      <SwipeableRow onDelete={function() { handleDelete(m); }}>
        ...
        >
          ...
          <View style={styles.movInfo}>
            <Text style={styles.movDesc} numberOfLines={1}>{catLabel}</Text>
            {m.subcategoria ? (
          ...
          {faturaData ? (
            <View style={styles.heroDates}>
              ...
              <Text style={styles.heroDue}>
                {'Venc. ' + formatDayMonth(faturaData.dueDate) + (dte > 0 ? ' (' + dte + ' dias)' : dte === 0 ? ' (hoje)' : '')}
              </Text>
            </View>
          ) : null}
        ...
        {hasBeneficio ? (
          <Glass style={styles.pontosCard}>
            <View style={styles.pontosRow}>
              <Ionicons
                name={cartao.tipo_beneficio === 'pontos' ? 'star-outline' : 'cash-outline'}
                size={18}
                color={C.yellow}
              />
              <Text style={styles.pontosLabel}>
                {cartao.programa_nome || (cartao.tipo_beneficio === 'pontos' ? 'Pontos' : 'Cashback')}
              </Text>
            </View>
            <Text style={styles.pontosValue}>
              {cartao.tipo_beneficio === 'pontos'
                ? Math.round(pontosValue).toLocaleString('pt-BR') + ' pontos este mês'
                : symb + ' ' + fmt(pontosValue) + ' este mês'}
            </Text>
          </Glass>
        ) : null}
              ...
              <Glass style={styles.payPanel}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.contaPills}>
                  {saldos.map(function(s) {
                    var selected = payConta === s.name;
                    return (
                      <TouchableOpacity
                        key={s.id || s.name}
                        style={[styles.contaPill, selected && styles.contaPillActive]}
                        activeOpacity={0.7}
                        onPress={function() { setPayConta(s.name); }}
                      >
        ...
        {!showManual ? (
          ...
          <Glass style={{ marginHorizontal: SIZE.padding, marginBottom: 10, padding: 14 }}>
            ...
            <Text style={{ fontSize: 11, color: C.dim, marginBottom: 10 }}>
              Registre apenas o valor total da fatura sem detalhar cada gasto.
            </Text>
            ...
            <TextInput
              style={{ height: 40, borderWidth: 1, borderColor: C.border, borderRadius: 10, paddingHorizontal: 12, color: C.text, fontFamily: F.body, fontSize: 13, backgroundColor: 'rgba(255,255,255,0.04)', marginBottom: 10 }}
              value={manualDesc}
              onChangeText={setManualDesc}
              placeholder={'Fatura ' + MESES[mes - 1] + '/' + ano}
              placeholderTextColor={C.dim}
            />
            <View style={{ flexDirection: 'row', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <TouchableOpacity
                style={{ backgroundColor: C.accent, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8, opacity: (parseBRVal(manualValor) > 0 && !savingManual) ? 1 : 0.4 }}
                activeOpacity={0.7}
              ...
              >
                {savingManual ? (
                  ...
                  <Text style={{ fontSize: 13, color: C.text, fontFamily: F.display }}>Lançar</Text>
                )}
              </TouchableOpacity>
            </View>
          </Glass>
...
var styles = StyleSheet.create({
  ...
  pontosRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 },
  pontosLabel: { fontFamily: F.body, fontSize: 13, color: C.text },
  pontosValue: { fontFamily: F.mono, fontSize: 14, color: C.yellow },
