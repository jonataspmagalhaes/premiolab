...
export default function EditOperacaoScreen(props) {
  ...
  var s1 = useState(op.tipo || 'compra'); var tipo = s1[0]; var setTipo = s1[1];
  var s2 = useState(op.categoria || 'acao'); var categoria = s2[0]; var setCategoria = s2[1];
  var s3 = useState(op.ticker || ''); var ticker = s3[0]; var setTicker = s3[1];
  var s4 = useState(String(op.quantidade || '')); var quantidade = s4[0]; var setQuantidade = s4[1];
  var s5 = useState(String(op.preco || '')); var preco = s5[0]; var setPreco = s5[1];
  var s6 = useState(op.corretora || ''); var corretora = s6[0]; var setCorretora = s6[1];
  var s7 = useState(String(op.custo_corretagem || '0')); var custoCorretagem = s7[0]; var setCustoCorretagem = s7[1];
  var s8 = useState(op.data || new Date().toISOString().split('T')[0]); var data = s8[0]; var setData = s8[1];
  var s9 = useState(false); var loading = s9[0]; var setLoading = s9[1];
  var s10 = useState(false); var showCustos = s10[0]; var setShowCustos = s10[1];
  ...
  var handleSave = async function() {
    ...
    try {
      ...
      if (result.error) {
        ...
        Alert.alert('Salvo!', 'Operacao atualizada.', [
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['top']}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Editar Operacao</Text>
          <View style={{ width: 32 }} />
        </View>
        ...
        <TextInput value={data} onChangeText={setData} placeholder="2025-02-13" placeholderTextColor={C.dim} style={styles.input} />
