...
function formatCurrency(value) {
  ...
  var str = (num * 100).toFixed(0);
  var centavos = parseInt(str);
...
export default function EditRendaFixaScreen(props) {
  var navigation = props.navigation;
  var rf = props.route.params.rf;
  var user = useAuth().user;

  var s1 = useState(rf.tipo || ''); var tipo = s1[0]; var setTipo = s1[1];
  var s2 = useState(rf.indexador || 'prefixado'); var indexador = s2[0]; var setIndexador = s2[1];
  var s3 = useState(String(rf.taxa || '')); var taxa = s3[0]; var setTaxa = s3[1];
  var s4 = useState(formatCurrency(rf.valor_aplicado)); var valorAplicado = s4[0]; var setValorAplicado = s4[1];
  var s5 = useState(isoToBr(rf.vencimento)); var vencimento = s5[0]; var setVencimento = s5[1];
  var s6 = useState(isoToBr(rf.data)); var dataAplicacao = s6[0]; var setDataAplicacao = s6[1];
  var s7 = useState(rf.emissor || ''); var emissor = s7[0]; var setEmissor = s7[1];
  ...
  var vencValido = isValidDate(vencimento);
  ...
  var handleSubmit = async function() {
    ...
    try {
      ...
      var isoData = dataAplicacao.length === 10 ? brToIso(dataAplicacao) : rf.data;
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <View style={styles.pillRow}>
          {TIPOS.map(function(t) {
            return (
              <Pill key={t.key} active={tipo === t.key} color={C.rf}
        ...
        <View style={styles.pillRow}>
          {INDEXADORES.map(function(idx) {
            return (
              <Pill key={idx.key} active={indexador === idx.key} color={idx.color}
        ...
        {idxInfo && <Text style={styles.hint}>{idxInfo.hint}</Text>}
        ...
        <View style={styles.row}>
          <View style={{ flex: 1 }}>
            ...
            <TextInput
              ...
              keyboardType="numeric"
              maxLength={10}
              style={[styles.input, vencimento.length === 10 && (vencValido ? { borderColor: C.green } : { borderColor: C.red })]}
            />
          </View>
          <View style={{ width: 12 }} />
          <View style={{ flex: 1 }}>
        ...
        <View style={styles.pillRow}>
          {CUSTODIAS.map(function(c) {
            return (
              <Pill key={c.key} active={custodia === c.key} color={C.rf}
...
var styles = StyleSheet.create({
  ...
  inputSuffix: { fontSize: 13, color: C.sub, fontFamily: F.mono },
  ...
  submitBtn: { backgroundColor: C.rf, borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginTop: 10 },
