...
export default function AddOpcaoScreen(props) {
  ...
  var s3 = useState(''); var ativoBase = s3[0]; var setAtivoBase = s3[1];
  var s10 = useState(''); var tickerOpcao = s10[0]; var setTickerOpcao = s10[1];
  var s4 = useState(''); var strike = s4[0]; var setStrike = s4[1];
  var s5 = useState(''); var premio = s5[0]; var setPremio = s5[1];
  var s6 = useState(''); var quantidade = s6[0]; var setQuantidade = s6[1];
  var s7 = useState(''); var vencimento = s7[0]; var setVencimento = s7[1];
  var s8 = useState(''); var corretora = s8[0]; var setCorretora = s8[1];
  ...
  var handleSubmit = async function() {
    if (!canSubmit) return;
    if (!dateValid) {
      Alert.alert('Data invalida', 'O vencimento deve ser uma data futura no formato DD/MM/AAAA.');
      return;
    }
    setLoading(true);
    try {
      ...
      if (result.error) {
        ...
        Alert.alert('Sucesso!', 'Opcao registrada.', [
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['top']}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Nova Opcao</Text>
          <View style={{ width: 32 }} />
        </View>
        ...
        <TouchableOpacity onPress={handleSubmit} disabled={!canSubmit || loading} activeOpacity={0.8} style={[styles.submitBtn, !canSubmit && { opacity: 0.4 }]}>
          {loading ? (
            ...
            <Text style={styles.submitText}>Registrar Opcao</Text>
          )}
        </TouchableOpacity>
